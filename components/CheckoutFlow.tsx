...
type Step = "cart-review" | "customer" | "envio" | "payment" | "confirmation";
...
export function CheckoutFlow() {
  ...
  const [confirmedPayment, setConfirmedPayment] = useState<PaymentMethod | "">("");
  const [confirmedShipping, setConfirmedShipping] = useState<ShippingMethod | "">("");

  const STEPS: Step[] = ["cart-review", "customer", "envio", "payment", "confirmation"];
        ...
        {step === "confirmation" && (
          <div className="text-center space-y-6 py-8">
            ...
            <div>
              ...
              <p className="text-gray-500 mt-2">Hemos recibido tu pedido. Nos pondremos en contacto contigo pronto.</p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm text-left space-y-3 max-w-sm mx-auto">
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-[#1e3a8a] font-mono mt-1">{orderId}</p>
              </div>
              <hr />
              <div className="text-sm space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium font-mono">{docTipo}-{docNumero}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Total USD</span>
                  <span className="font-bold text-[#1e3a8a]">${confirmedSubtotal.toFixed(2)}</span>
                </div>
                {confirmedBcvRate && confirmedPayment && BS_METHODS.includes(confirmedPayment as PaymentMethod) && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Total Bs (BCV)</span>
                    <span className="font-bold text-[#1e3a8a]">Bs. {formatBs(confirmedSubtotal * confirmedBcvRate.tasa)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-500">Envío</span>
                  <span className="font-medium">{SHIPPING_OPTIONS.find((o) => o.id === confirmedShipping)?.label}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Pago</span>
                  <span className="font-medium">{PAYMENT_OPTIONS.find((o) => o.id === confirmedPayment)?.label}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-sm mx-auto">
              <a
                href={buildWhatsAppUrl(
                  orderId, confirmedItems, confirmedSubtotal,
                  buildCustomer(),
                  confirmedPayment as PaymentMethod,
                  confirmedShipping,
                  confirmedBcvRate
                )}
                target="_blank"
